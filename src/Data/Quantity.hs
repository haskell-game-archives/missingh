...
quantifyNum opts n = first head $ quantifyNums opts [n]
...
quantifyNums opts (headnum : xs) = (map procnum (headnum : xs), suffix)
  where
    ...
    finderfunc (x, _) = fromIntegral (base opts) ** fromIntegral x <= abs number
    ...
    procnum n = (fromRational . toRational $ n) / (fromIntegral (base opts) ** fromIntegral usedexp)
...
renderNum ::
  ...
  a ->
  String
renderNum opts prec number = printf ("%." ++ show prec ++ "g") num ++ [suffix]
  where
    (num, suffix) = quantifyNum @a @Double opts number
...
renderNums ::
  ...
  -- | Result
  [String]
renderNums opts prec numbers = map printit convnums
  where
    printit num = printf ("%." ++ show prec ++ "f") num ++ [suffix]
    (convnums, suffix) = quantifyNums @a @Double opts numbers
...
parseNum opts insensitive inp =
  ...
  where
    suffixMap =
      zip
        (map caseTransformer . suffixes $ opts)
...
parseNumInt opts insensitive inp =
  case parseNum @Double opts insensitive inp of
    Left x -> Left x
