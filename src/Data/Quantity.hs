...
quantifyNums opts (headnum : xs) =
  ...
  where
    ...
    finderfunc (x, _) =
      fromIntegral (base opts) ** fromIntegral x <= abs number
    ...
    procnum n =
      (fromRational . toRational $ n) / (fromIntegral (base opts) ** fromIntegral usedexp)
...
renderNum opts prec number = printf ("%." ++ show prec ++ "g") num ++ [suffix]
  where
    (num, suffix) = quantifyNum opts number :: (Double, Char)
...
renderNums opts prec numbers =
  ...
  where
    ...
    (convnums, suffix) = quantifyNums opts numbers :: ([Double], Char)
