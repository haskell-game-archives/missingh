...
timegm ct = timeDiffToSecs $ diffClockTimes (toClockTime ct) (toClockTime epoch)
...
timelocal ct = do
  guessct <- toCalendarTime guesscl
  let newct = ct {ctTZ = ctTZ guessct}
  return $ timegm newct
  where
    guesscl = toClockTime ct
...
timeDiffToSecs td =
  fromIntegral (tdSec td)
    + 60
      * ( fromIntegral (tdMin td)
            + 60
              * ( fromIntegral (tdHour td)
                    + 24
                      * ( fromIntegral (tdDay td)
                            + 30
                              * ( fromIntegral (tdMonth td)
...
epochToClockTime x = TOD seconds secfrac
  where
    ratval = toRational x
