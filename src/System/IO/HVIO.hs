...
instance HVIO Handle where
  ...
  vIsEOF = hIsEOF
  vShow = hShow
  vMkIOError h et desc mfp = mkIOError et desc (Just h) mfp
  vGetChar = hGetChar
  vGetLine = hGetLine
...
newMemoryBuffer ::
  ...
  (String -> IO ()) ->
  IO MemoryBuffer
newMemoryBuffer initval closefunc = do
  ref <- newIORef (True, (0, initval))
