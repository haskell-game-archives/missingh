...
recurseDir fs x = recurseDirStat fs x <&> map fst
...
recurseDirStat h fn = do
  fs <- vGetSymbolicLinkStatus h fn
  if withStat fs vIsDirectory
    then do
      dirc <- vGetDirectoryContents h fn
      let contents =
            map ((fn ++ [pathSeparator]) ++) $
              filter (\x -> x /= "." && x /= "..") dirc
...
lsl fs fp =
  let showmodes mode =
        let i m = (intersectFileModes mode m /= 0)
         in [ if i ownerReadMode then 'r' else '-',
              if i ownerWriteMode then 'w' else '-',
              if i setUserIDMode
                then 's'
                else if i ownerExecuteMode then 'x' else '-',
              if i groupReadMode then 'r' else '-',
              if i groupWriteMode then 'w' else '-',
              if i setGroupIDMode
                then 's'
                else if i groupExecuteMode then 'x' else '-',
      ...
      showentry origdir fh (state, fp') =
        case state of
          HVFSStatEncap se ->
            let typechar
                  | vIsDirectory se = 'd'
                  | vIsSymbolicLink se = 'l'
                  | vIsBlockDevice se = 'b'
                  | vIsCharacterDevice se = 'c'
                  | vIsSocket se = 's'
                  | vIsNamedPipe se = 's'
                  | otherwise = '-'
                ...
                datestr c =
                  formatCalendarTime
                    ...
                    c
             in do
                  c <- toCalendarTime clocktime
                  linkstr <-
                    if vIsSymbolicLink se
                      then do
                        sl <- vReadSymbolicLink fh (origdir </> fp')
                        return $ " -> " ++ sl
                      else return ""
                  return $
                    printf
                      ...
                      linkstr
   in do
        c <- vGetDirectoryContents fs fp
        pairs <-
          mapM
            ( \x -> do
                ss <- vGetSymbolicLinkStatus fs (fp </> x)
                return (ss, x)
            )
            c
